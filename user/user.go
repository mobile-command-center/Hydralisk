...
func (u *User) Do(v url.Values, data UserData) (status int, err error) {
	...
	defer u.Logout(u.url[LogoutLevel])
	return
}
...
func (u *User) Login(loginURL string) (int, error) {
	...
	set_cookie_url, err := url.Parse(loginURL)
	if err != nil {
		log.Fatal(err)
	}

	cookies := u.client.Jar.Cookies(set_cookie_url)
	log.Printf("%v\n", cookies)
...
func (u *User) Logout(logoutURL string) (int, error) {
	...
	set_cookie_url, err := url.Parse(logoutURL)
	if err != nil {
		log.Fatal(err)
	}
...
func (u *User) Register(registerURL string, v url.Values, user UserData) (int, error) {
	set_cookie_url, err := url.Parse(registerURL)
	if err != nil {
		log.Fatal(err)
	}
